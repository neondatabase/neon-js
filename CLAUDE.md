...
- On same-origin middleware/proxy login redirects, retain configured `loginUrl` query entries and append incoming entries only for keys not already configured (including **`neon_auth_session_verifier`** when its key is not configured), even when the OAuth challenge cookie is absent or verifier exchange fails. Never copy request query parameters to an external-origin `loginUrl`; don't allow the protected request through instead.
- Prefer defaulting **`cookies.sameSite`** to **`Lax`** (a single simple global default) over `Strict` or per-cookie-name special-casing, while still honoring an explicit `strict`/`lax`/`none`; `Strict` blocks the cross-site top-level OAuth callback.
- When behavior depends on upstream Neon Auth cookies, **verify the actual upstream `Set-Cookie` output** (e.g. a redacted diagnostic capture across social/password sign-in, verifier exchange, sign-out) rather than inferring cookie attributes from neon-js fixtures.
- Keep feature PRs free of package version bumps; the **prepare-release workflow** owns package version and tag updates.
- Coordinate **`better-auth` dependency upgrades** with the Neon Auth server version rather than bumping the SDK independently solely to clear an audit.
...
- The shared proxy response handler **`prepareResponseHeaders()`** in `packages/auth/src/server/proxy/response.ts` rewrites **`SameSite` on every upstream `Set-Cookie`** to `effectiveSameSite` (= explicit `cookies.sameSite`, otherwise **`'lax'`**), strips **`Partitioned`**, and applies the configured `Domain`; middleware forwards the resulting cookies.
- Upstream Neon Auth emits **`session_token`** and the OAuth **`session_challenge`** (legacy misspelling **`session_challange`**, both accepted) as **`SameSite=None; Partitioned`**. `Strict` blocks the challenge cookie on the cross-site top-level OAuth callback (Safari/Chrome), so it must be at most `Lax` to survive; the Safari compatibility path strips `Partitioned` and rewrites `SameSite`. On absent-challenge or failed-exchange login redirects, same-origin middleware retains configured `loginUrl` query entries and appends only non-colliding request entries; external-origin login URLs receive no request query parameters. A successful exchange removes the consumed **`neon_auth_session_verifier`**.
