...
- **Bundled Nuxt 4 adapter**: New **`@neondatabase/auth/nuxt`** client entry uses Better Auth's Vue client for same-origin `/api/auth` calls, including the new **`BetterAuthVueAdapter`** and **`VueBetterAuthClient`** type. New **`@neondatabase/auth/nuxt/server`** entry provides `createNeonAuth(config)` with `withEvent(event)`-bound server methods, a Nitro catch-all `handler()`, and route-protection `middleware({ loginUrl, protectedRoutes })`. The H3 v1 integration preserves raw request bodies, duplicate query values, and multiple `Set-Cookie` headers while explicitly binding each server proxy to its `H3Event`.
