...
- **Nuxt 4 adapter**: Adds **`@neondatabase/auth/nuxt`** with a Better Auth Vue client for same-origin `/api/auth` calls. **`@neondatabase/auth/nuxt/server`** provides `createNeonAuth(config)`, event-bound server methods, a Nitro catch-all handler and route-protection middleware. Each proxy is tied to its `H3Event`. The handler forwards raw request bodies and duplicate query values while keeping `Set-Cookie` headers separate.
