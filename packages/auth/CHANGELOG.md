...
- **Expo client support**: New `@neondatabase/auth/expo` subpath exports `BetterAuthExpoAdapter`, which leaves web requests unchanged and adds SecureStore-backed cookies, native OAuth sessions, and `neon_auth_session_verifier` exchange on Expo native. Internal client identifiers no longer require a global Web Crypto polyfill.
