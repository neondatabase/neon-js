...
- **Next.js OAuth login redirects**: Unauthenticated middleware redirects now preserve every incoming query parameter, including repeated and colliding keys, while retaining parameters configured on `loginUrl`. Successful OAuth exchanges still remove the consumed `neon_auth_session_verifier` from the post-exchange redirect.
